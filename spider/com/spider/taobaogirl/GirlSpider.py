...
def main():
    driver = webdriver.PhantomJS(executable_path='E:/taobao/shiyanlou/phantomjs/bin/phantomjs')#浏览器的地址
    ...
    while(i<5):
        ...
        path = 'E:/taobao/shiyanlou/photo/'+contents[i][0][0]
        mkdir(path)#建立相应文件夹
    ...
    for imageUrl in imagesUrl:#将封面图片存入对应文件夹中
        ...
        html = urlopen(url)
        data = html.read()
        fileName = 'E:/taobao/shiyanlou/photo/mm'+str(number)+'.jpg'
...
def getperMMpageImg(MMURL,MMpath):
    owndriver = webdriver.PhantomJS(executable_path='E:/taobao/shiyanlou/phantomjs/bin/phantomjs')
    owndriver.get(MMURL)
    print("[*]Opening.....MM....................")
