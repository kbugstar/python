#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2016/12/28 0028 11:40
# @Author  : Aries
# @Site    : 
# @File    : isSold_pred.py
# @Software: PyCharm
'''
以下就是我们使用sklearn中的SGDClassifier实现拍卖是否成功的模型训练代码
'''
...
可看到SGDClassifier的训练效果还不错，我们也可以通过scikit-learn中封装的一些降维方法，将数据可视化,这里我们使用的三种方法进行降维 ———— Random Projection，PCA 和T-SNE embedding。
'''
...
'''
    从以上三幅图中，我们可以根据数字0和1的重叠情况，判读出我们的数据可区分度并不是特别大。因此我们的训练效果也没有达到特别好，从这方面也可以看出我们这里用来训练分类器的特征并不是利用得特别好。有兴趣的同学可以多了解下特征工程，或者根据我们上面绘制的热度图挑选一些特征进行分类器训练，查看训练效果。
